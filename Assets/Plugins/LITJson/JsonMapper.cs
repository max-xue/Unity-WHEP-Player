...
namespace LITJson
{
	...
	public class JsonMapper
	{
		...
		private static void RegisterBaseExporters()
		{
			...
			//baseExportTable[typeof(int)] = delegate (object obj, JsonWriter writer) {
			//	writer.Write(Convert.ToInt32((int)obj));
			//};
		}
		...
		private static void RegisterBaseImporters()
		{
			...
			//});
			//RegisterImporter(baseImportTable, typeof(int), typeof(string), delegate (object input) {
		...
		private static void WriteValueProp(object obj, JsonWriter writer, bool privateWriter, int depth)
		{
			if (depth > maxNestingDepth)
			{
				throw new JsonException(string.Format("Max allowed object depth reached while trying to export from type {0}", obj.GetType()));
			...
			foreach (string property in tdata.Properties.Keys)
			{
				...
				//todo ??暂时去除类属性的序列化操作,因为静态数据里面带有不需要序列化的属性
				else
				{
					PropertyInfo info = (PropertyInfo)pdata.Info;
					if (info.CanRead)
		...
		public static string ToJsonProp(object obj)
		{
			return ToJson(obj, false);
		}
		...
		public static string ToJsonProp(object obj, bool prettyPrint)
		{
			JsonWriter writer = new JsonWriter();
			writer.PrettyPrint = prettyPrint;
			WriteValueProp(obj, writer, true, 0);
			return writer.ToString();
		}
